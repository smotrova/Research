...
COMPLEX(8), INTENT(IN), DIMENSION (N)		:: y

COMPLEX(8), INTENT(OUT), DIMENSION (N)		:: A_p, B_p
...
COMPLEX(8), INTENT(IN), DIMENSION (N)		:: A_p
...
COMPLEX(8), INTENT(IN), DIMENSION (N)	:: B_p
...
DO s = 1, N_res
	...
	DO p = 1, N	
		...
		sum_p = sum_p + B_p(p)*Hnk(p)*dsin(p*fi_s)
...
!Postroenie konturov resonatorov
	...
	DO s = 1, N_res
		...
		DO fi = 0, 360, 0.1
			...
			WRITE(21,'(2X,E13.6, 3X,E13.6 )'), sngl(x_s(s)), sngl(y_s(s))
