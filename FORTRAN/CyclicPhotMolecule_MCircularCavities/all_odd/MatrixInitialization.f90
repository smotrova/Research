...
REAL(8)					:: l, teta_plus, teta_minus, ro_1s
	...
	DO m=1,N
		...
		DO p=1,N
			...
			DO s = 2, N_res
				...
				sum = (-1)**p*h_minus*dcos(p*teta_plus - m*teta_minus) - h_plus*dcos(p*teta_plus + m*teta_minus) + sum

			END DO


			IF (abs(m-p) .ne. 0) A(m,p) = (-1)**m*J(m)*VJF*sum
			IF (abs(m-p) .eq. 0) A(m,p) = 1.0D0 + (-1)**m*J(m)*VJF*sum
