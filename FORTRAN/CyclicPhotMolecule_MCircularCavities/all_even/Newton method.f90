...
REAL(8), PARAMETER		:: e_newt = 1.0D-6						!Accuracy of Newton method
...
REAL(8), PARAMETER, DIMENSION(1:nn) :: h=(/1.D-4, 1.D-7/)		! Shag po peremennoi kapa, gama dla	chislennogo nahojdeniya pervih proizvodnih po kapa, gama sootvetstvenno
...
!				DO WHILE  (v>=e_newt)  
				...
				DO WHILE  (v1 > e_newt .or. v2 > e_newt)  
					...
					A(1,1) = (f1(kapa1, gama00, N) - f1(kapa2, gama00, N))/h(1)
					A(1,2) = (f1(kapa00, gama1, N) - f1(kapa00, gama2, N))/h(2)
					A(2,1) = (f2(kapa1, gama00, N) - f2(kapa2, gama00, N))/h(1)
					A(2,2) = (f2(kapa00, gama1, N) - f2(kapa00, gama2, N))/h(2)
...
!					IF (ERROR1) print*, 'ERROR FROM GAUSS'
					...
					v1=dabs((x(1)-kapa00)/x(1))
					v2=dabs((x(2)-gama00)/x(2))

!					v=dsqrt(v1*v1+v2*v2)/v
