...
DO s = 1, N_res
	...
	DO p = 0, N	
		...
		sum_p = sum_p + (-1)**(s-1)*B_p(p)*Hnk(p)*dcos(p*fi_s)
...
USE init_data_class, ONLY : PI, zeta, N_res, alfa
			...
			DO z = -2 - ro, 2 + ro, 0.05
				...
				DO s = 1, N_res
					...
					IF ( r_s(s) <= 1.0D0 ) THEN
						...
						u = Ez_in (kapa, gama, N, (-1)**(s-1)*A_p, r_s(s), fi_s(s))
...
!				WRITE(7,'(2X,E13.6, 3X,E13.6, 3X,E13.6 )'), sngl(z), sngl(y), sign(cdabs(u)/E_max, u1)
