...
REAL(8), PARAMETER, DIMENSION(1:nn) :: h=(/1.D-9, 1.D-9/)		! Shag po peremennoi kapa, gama dla	chislennogo nahojdeniya pervih proizvodnih po kapa, gama sootvetstvenno
...
!=============================================================================================
				...
				DO WHILE  (v>=e_newt)  
					...
					A(1,1) = (f1(kapa1, gama00) - f1(kapa2, gama00))/h(1)
					A(1,2) = (f1(kapa00, gama1) - f1(kapa00, gama2))/h(2)
					A(2,1) = (f2(kapa1, gama00) - f2(kapa2, gama00))/h(1)
					A(2,2) = (f2(kapa00, gama1) - f2(kapa00, gama2))/h(2)


!					A(1,1) = ( -f1(kapa00+2*h(1), gama00) + 8*f1(kapa00+h(1), gama00)-8*f1(kapa00-h(1), gama00) + f1(kapa00-2*h(1), gama00) )/12/h(1)
!					A(1,2) = ( -f1(kapa00, gama00+2*h(2)) + 8*f1(kapa00, gama00+h(2))-8*f1(kapa00, gama00-h(2)) + f1(kapa00, gama00-2*h(2)) )/12/h(2)
!					A(2,1) = ( -f2(kapa00+2*h(1), gama00) + 8*f2(kapa00+h(1), gama00)-8*f2(kapa00-h(1), gama00) + f2(kapa00-2*h(1), gama00) )/12/h(1)
!					A(2,2) = ( -f2(kapa00, gama00+2*h(2)) + 8*f2(kapa00, gama00+h(2))-8*f2(kapa00, gama00-h(2)) + f2(kapa00, gama00-2*h(2)) )/12/h(2)


					b(1) = -f1(kapa00,gama00) + a(1,1)*kapa00 + a(1,2)*gama00
					b(2) = -f2(kapa00,gama00) + a(2,1)*kapa00 + a(2,2)*gama00
